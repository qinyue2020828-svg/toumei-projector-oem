...
const products = [
  { name: "C Series", tag: "Pocket Outdoor", image: "/products/c-series.jpg", models: "C900 · C1000 · C2000", specs: "200 ANSI · 8000mAh · DLP-LINK 3D", detail: "One-hand portable projectors with Type-C power, 1.07:1 short throw and up to 190 minutes of battery life." },
  { name: "M Series", tag: "Outdoor 1080P", image: "/products/m-series.jpg", models: "M1 · M2", specs: "1080P · 300 ANSI · 15000mAh", detail: "Cup-sized DLP projectors with LiDAR autofocus, real-time keystone correction and an ultra-quiet 28dB cooling system." },
  { name: "S Series", tag: "Gimbal & Education", image: "/products/s-series.jpg", models: "S7 · S8 · S11 · S13", specs: "Up to 1000 ANSI · 20800mAh", detail: "Flexible gimbal projectors for home cinema, camping and smart education, available with triple-laser illumination." },
  { name: "X Series", tag: "Flagship Brightness", image: "/products/x-series.jpg", models: "X5 · X6", specs: "1080P · 1600 ANSI · HDR10", detail: "High-brightness DLP systems for meeting rooms and home theaters, with fast laser focus and wide-gamut imaging." },
  { name: "Q Series", tag: "LCD Home Theater", image: "/products/q-series.jpg", models: "Q3 · Q7", specs: "1080P · Up to 800 ANSI · Autofocus", detail: "Sealed optical engines, coated glass lenses and cinema-grade sound for dependable long-session viewing." },
  { name: "UST Series", tag: "Ultra Short Throw", image: "/products/ust-series.jpg", models: "S1 · S2", specs: "0.18:1 throw · 1000 ANSI · 110% BT.2020", detail: "True 1080P ultra-short-throw projectors, including a triple-laser model that creates 100 inches from just 23cm." },
];
    ...
    <main className="overflow-hidden bg-white text-slate-950">
        ...
        <div className="mx-auto grid min-h-[700px] max-w-7xl items-center gap-12 px-5 py-20 lg:grid-cols-[1.08fr_.92fr] lg:px-8">
          ...
          <div className="relative hidden min-h-[500px] lg:block" aria-label="Featured Toumei projectors">
            <div className="absolute left-8 top-7 w-[78%] rotate-2 overflow-hidden rounded-[2rem] border border-white/10 bg-white p-4 shadow-2xl shadow-black/30 transition duration-500 hover:rotate-0 hover:scale-[1.02]">
              <Image src="/products/s-series.jpg" alt="Toumei S Series gimbal projectors" width={900} height={600} priority className="aspect-[4/3] w-full rounded-2xl object-cover" />
            </div>
      ...
      <section id="products" className="mx-auto max-w-7xl px-5 py-24 lg:px-8">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {products.map((p, i) => <article key={p.name} className="product-card group overflow-hidden rounded-3xl border border-slate-200 bg-white">
            <div className="relative aspect-[3/2] overflow-hidden bg-slate-100"><Image src={p.image} alt={`Toumei ${p.name} projector family`} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover transition duration-700 group-hover:scale-105" /><span className="absolute left-4 top-4 rounded-full bg-[#09274b]/90 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wider text-white backdrop-blur">{p.tag}</span></div>
            <div className="p-6"><div className="flex items-center justify-between"><h3 className="text-2xl font-black text-[#08264a]">{p.name}</h3><span className="text-sm font-black text-orange-500">0{i+1}</span></div><p className="mt-2 text-sm font-bold text-slate-500">{p.models}</p><p className="mt-4 text-sm leading-6 text-slate-600">{p.detail}</p><div className="mt-5 border-t border-slate-100 pt-4 text-xs font-bold uppercase tracking-wider text-orange-600">{p.specs}</div></div>
          </article>)}
        </div>
      </section>
      ...
      <section id="about" className="mx-auto grid max-w-7xl items-center gap-14 px-5 py-24 lg:grid-cols-2 lg:px-8">
        <div className="relative"><Image src="/products/x-series.jpg" alt="Toumei projector manufacturing portfolio" width={900} height={600} className="w-full rounded-[2rem] object-cover shadow-2xl" /><div className="absolute -bottom-6 -right-2 rounded-2xl bg-orange-500 p-6 text-white shadow-xl sm:right-8"><strong className="block text-3xl font-black">10M+</strong><span className="text-xs font-bold uppercase tracking-wider">R&D investment</span></div></div>
        <div><p className="eyebrow">About Toumei</p><h2 className="section-title">Engineering trust into every image.</h2><p className="mt-6 leading-8 text-slate-600">Shenzhen Toumei Technology Co., Ltd. is one of China’s early high-tech enterprises in DLP smart projection. Since 2013, we have combined production, sales and R&D to deliver intelligent projection and 3D imaging solutions worldwide.</p><p className="mt-4 leading-8 text-slate-600">Our operating quality and environmental management systems follow ISO 9001/14000 standards, with customer satisfaction, continuous innovation and precision manufacturing at the center.</p><div className="mt-8 grid grid-cols-3 gap-3 text-center">{[['Integrity','Accountable'],['Dedication','Committed'],['Appreciation','Collaborative']].map(([a,b]) => <div key={a} className="rounded-xl bg-slate-50 p-4"><strong className="block text-sm text-[#08264a]">{a}</strong><span className="text-xs text-slate-500">{b}</span></div>)}</div></div>
      </section>
